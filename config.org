    ...
    (let* ((path-s (if default-directory (ava/path-slug default-directory) ""))
	       ...
	       (shell-command-buffer-name-async
	        (format
	         "*async-shell %s*"
	         (string-trim
	          (substring buffname 0 (min (length buffname) 150))))))
      (apply orig-fun args)))
...
(use-package gptel
  ...
  (defun ava/init-gptel-mode ()
    (visual-line-mode)
    (setq-local gptel--system-message "You are a computer programmer's assistant living inside Emacs. Your responses are always concise, succinct, and to the point. You never apologize for anything as that is a waste of time."))
