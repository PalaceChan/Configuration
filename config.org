# This first block tangles to line 1 of config.el. It keeps the old dynamic-binding
# semantics explicitly; Emacs 31 warns when a loaded file has no cookie.
...
#+BEGIN_SRC emacs-lisp
  (use-package saveplace
    :preface
    (defun ava/save-place-recenter (&rest _)
      "Recenter after `save-place' restores point."
      (when buffer-file-name
        (ignore-errors (recenter))))
    :config
    (advice-remove 'save-place-find-file-hook #'ava/save-place-recenter)
    (advice-add 'save-place-find-file-hook :after #'ava/save-place-recenter)
    (save-place-mode))
#+END_SRC
*** highlighting
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  (use-package winner
    :preface
    (defun ava/toggle-delete-other-windows ()
    ...
    :bind (("M-N" . winner-redo)
           ("M-P" . winner-undo)
           ("C-x 1" . ava/toggle-delete-other-windows))
    :config
...
(use-package compile
  :preface
  (defun ava/compilation-buffer-name (mode-name)
    "Name compilation buffers as *<mode> <project> - <tool action>*."
    (require 'subr-x)
    (let* (( proj (and (fboundp 'projectile-project-name)
                       (ignore-errors (projectile-project-name))))
           (dir (file-name-nondirectory (directory-file-name default-directory)))
           (ctx (or proj dir ""))
           (cmd (string-trim (or compile-command "")))
           ;; grab first two tokens: e.g. "bazel build", "bazel test", "ninja -C"
           (kind (if (string-match "\\`\\([^ ]+\\(?: [^ ]+\\)?\\)" cmd)
                     (match-string 1 cmd)
                   ""))
           (suffix (if (> (length kind) 0) (concat " - " kind) "")))
      (format "*%s %s%s*" mode-name ctx suffix)))
  :custom
  (compilation-buffer-name-function #'ava/compilation-buffer-name)
...
(use-package copilot
  :ensure t
  :preface
  (defvar ava/copilot-enabled-projects '("insert_project_names"))
  (defun ava/copilot--project-enabled-p ()
    (when (fboundp 'projectile-project-name)
      (let ((name (projectile-project-name)))
        (and name (member name ava/copilot-enabled-projects)))))
  (defun ava/copilot-ensure ()
    "Enable copilot in supported modes for allowed projects."
    (when (and (derived-mode-p 'c++-mode 'c++-ts-mode 'python-mode 'python-ts-mode)
               (ava/copilot--project-enabled-p))
      (copilot-mode 1)))
  (defun ava/copilot-quit ()
    "Clear copilot ghost text on quit."
  ...
  (dolist (hook '(python-mode-hook
                  ...
                  c++-mode-hook
                  c++-ts-mode-hook))
    (add-hook hook #'ava/copilot-ensure))
...
(use-package eca
  ...
  :preface
  (defun ava/eca-authinfo-password (host)
    "Return auth-source password for HOST using user \"apikey\"."
    (require 'auth-source)
    (auth-source-pick-first-password :host host :user "apikey"))
  :init
  (setenv "OPENAI_API_KEY"    (ava/eca-authinfo-password "api.openai.com"))
  (setenv "ANTHROPIC_API_KEY" (ava/eca-authinfo-password "api.anthropic.com"))
  (setenv "OPENROUTER_API_KEY" (ava/eca-authinfo-password "api.openrouter.com"))
  :config
  (defun ava/eca-chat-display-in-new-window (buffer _alist)
    "Display BUFFER in a newly split regular window for ECA chat."
    (let ((window (or (ignore-errors (split-window-right))
                      (ignore-errors (split-window-below)))))
      (unless window
        (user-error "Unable to create a new window for ECA chat"))
      (set-window-buffer window buffer)
      window))
  (defun ava/eca-chat-new-in-new-window (orig-fn &rest args)
    "Make `eca-chat-new' open in a freshly split regular window."
    (let ((display-buffer-overriding-action
           '((ava/eca-chat-display-in-new-window)
             (inhibit-same-window . t))))
      (apply orig-fn args)))
  (unless (advice-member-p #'ava/eca-chat-new-in-new-window 'eca-chat-new)
    (advice-add 'eca-chat-new :around #'ava/eca-chat-new-in-new-window))
  :bind (("C-c k e" . eca)
         ("C-c k ." . eca-transient-menu)
         ("C-c k c" . eca-chat-toggle-window)
         ("C-c k k" . eca-stop)
         ("C-c k r" . eca-restart)))
#+end_src
...
#+BEGIN_SRC emacs-lisp
(use-package fleet
  :if (file-directory-p "~/development/fleet/lisp")
  ...
  :commands (fleet-dashboard fleet-new fleet-park fleet-destroy fleet-doctor
             ...
             fleet-timeline fleet-stats fleet-task-close)
...
(use-package gptel
  ...
  (ava/add-or-update-openai-model 'gpt-5.5
                                  '(:description "Reasoning model designed to solve hard problems across domains"
                                                 :context-window 400
                                                 :input-cost 1.25
                                                 :output-cost 10
                                                 :cutoff-date "2024-10"
                                                 :request-params (:model "gpt-5.5" :stream t :reasoning_effort "low")))
  (ava/add-or-update-openai-model 'gpt5.5-thinking
                                  '(:description "Reasoning model designed to solve hard problems across domains"
                                                 :context-window 400
                                                 :input-cost 1.25
                                                 :output-cost 10
                                                 :cutoff-date "2024-10"
                                                 :request-params (:model "gpt-5.5" :stream t :reasoning_effort "xhigh")))
  (setq ava/openai-model-list
        (let ((desired-models '("gpt-5.5" "gpt5.5-thinking"))
              (result '()))
...
(use-package helm
  ...
  :bind (
         ...
         ("C-c h l" . helm-locate)
         ("C-c h g s" . helm-grep-do-git-grep)
  ...
  (defvar ava/helm-mini-base-sources helm-mini-default-sources)
  (defun ava/reset-helm-buffer-max-length (&rest ignore) (setq helm-buffer-max-length nil))
  (defun ava/reset-helm-mini-default-sources (&rest ignore)
    (let ((sources ava/helm-mini-base-sources))
      (when (and (require 'helm-ls-git nil t)
                 (fboundp 'helm-ls-git-root-dir)
                 (ignore-errors (helm-ls-git-root-dir)))
        ;; `helm-ls-git' only builds `helm-source-ls-git' when its main
        ;; command runs, so do ithere when `helm-mini' wants to reuse it.
        (unless helm-source-ls-git
          (setq helm-source-ls-git
                (and (fboundp 'helm-ls-git-build-ls-git-source)
                     (helm-ls-git-build-ls-git-source))))
        (when (and helm-source-ls-git
                   (not (memq 'helm-source-ls-git sources)))
          (setq sources (append sources '(helm-source-ls-git)))))
      (setq helm-mini-default-sources sources)))
  (advice-add 'helm-mini :before #'ava/reset-helm-buffer-max-length)
  (advice-add 'helm-mini :before #'ava/reset-helm-mini-default-sources)
...
(use-package helm-shell-history
  ...
  :after term vterm
  :custom
  (helm-shell-history-file "~/.bash_eternal_history")
  (helm-shell-history-fast-parser "~/development/helm-shell-history/src/parse_history")
  :config
...
(use-package hydra
  ...
  (global-set-key
   ...
   (defhydra hydra-windows (:hint nil)
     ...
     ("\\" rotate-frame-anticlockwise "anticlockwise")
     ("v" flip-frame "flip")
     ("f" flop-frame "flop")
...
(use-package magit
  ...
  (magit-diff-refine-hunk 'all) ;nil for extra perf
  ...
  ;; (remove-hook 'magit-status-section-hook 'magit-insert-tags-header)
  ;; (remove-hook 'magit-status-section-hook 'magit-insert-status-headers)
  ;; (remove-hook 'magit-status-section-hook 'magit-insert-untracked-files)
  ;; (remove-hook 'magit-status-section-hook 'magit-insert-unpushed-to-pushremote)
  ;; (remove-hook 'magit-status-section-hook 'magit-insert-unpulled-from-upstream)
  ;; (remove-hook 'magit-status-section-hook 'magit-insert-unpulled-from-pushremote)
  ;; (remove-hook 'magit-status-section-hook 'magit-insert-unpushed-to-upstream-or-recent)
  (add-hook 'magit-status-sections-hook #'magit-insert-worktrees t)
  (require 'transient)
  (transient-define-prefix ava/magit-tools ()
    "Ava Magit Tools."
    ["Misc"
     ("f" "Format staged files" ava/format-staged)])
  (transient-append-suffix 'magit-dispatch "!" '("#" "Ava tools" ava/magit-tools))
  ;; if we end up with other tools like open on bitbucket/github, copy link or need a make pull request without forge
  ;; (transient-append-suffix 'magit-file-dispatch "B" '("#" "Ava tools" ava/magit-tools))
  )
#+END_SRC
...
(use-package projectile
  ...
  (projectile-mode +1)
  :hook
  (emacs-startup . projectile-known-projects)
  :diminish projectile-mode
  :custom
  (projectile-enable-caching t)
  (projectile-auto-discover nil)
  (projectile-per-project-compilation-buffer t)
  (projectile-project-root-functions '(projectile-root-local projectile-root-marked))
  ...
  :bind
  (:map projectile-command-map ("R" . ava/generate-tags)))
...
#+BEGIN_SRC emacs-lisp
(defun ava/rvterm (&optional host)
  "Start a dir-tracking vterm on HOST (prompt if nil)."
  (interactive
   (list (read-string "Host: " (or (thing-at-point 'symbol) ""))))
         ...
         (setup
          (mapconcat
           ...
           '("function vterm_printf() { printf \"\\e]%s\\e\\\\\" \"$1\"; }"
             ...
             "if [[ -n \"$PROMPT_COMMAND\" ]]; then PROMPT_COMMAND=\"vterm_prompt_end; $PROMPT_COMMAND\"; else PROMPT_COMMAND=\"vterm_prompt_end\"; fi"
             "export PROMPT_COMMAND")
           "\n"))
         (bigcmd (concat sshcmd "\n" setup "\n")))
    (vterm bname)
    (switch-to-buffer bname)
    (vterm-send-string bigcmd)
    (vterm-send-return)))
#+END_SRC
