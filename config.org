...
use ~M-x copilot-login~ to authenticate, =copilot-mode= would turn it on automatically but then bindings can shadow company bindings
and the disable-predicates lists aren't helping that much so instead use ~M-C-<return>~ to trigger it manually
