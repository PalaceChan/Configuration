...
#+END_SRC
*** re-builder
settings and query replace from there helper
(from https://blog.echosa.net/blog/2012/06/26/re-builder-query-replace-this/)
#+begin_src emacs-lisp
  (use-package re-builder
    :config
    (setq reb-re-syntax 'string)
    (defun ava/reb-query-replace-this-regxp (replace)
    ...
    (define-key reb-mode-map "\C-c\M-%" 'ava/reb-query-replace-this-regxp))
#+end_src
* active packages
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package helm-ag :ensure t :custom (helm-ag-insert-at-point 'symbol))
