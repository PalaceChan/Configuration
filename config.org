...
#+END_SRC
(consider the defadvice to xref-find-definitions in https://www.emacswiki.org/emacs/TagsFile
if wanting to do different TAGS on a per projectile level or fallback to a global default)
*** semantic mode
